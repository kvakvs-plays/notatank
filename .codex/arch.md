...
Reminder overlays are profile-scoped under `profile.overlays`. `src/Reminders.lua` owns two movable frames: target debuffs and warrior shouts. Target reminders are passive texture icons that can update during combat and check the current hostile target for enabled class debuffs: warrior Thunder Clap and Demoralizing Shout; paladin Judgement variants; bear druid Faerie Fire, Demoralizing Roar, and Mangle variants. Warrior shout reminders are also passive texture icons that update during combat while watching Battle Shout and Commanding Shout on the player, using aura caster data when available and recent player spellcast tracking as a best-effort ownership fallback.
