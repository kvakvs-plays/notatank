...
`src/Auras.lua` exposes `addon.TargetAuras`, a metatable-style snapshot created from a WoW unit token such as `target` or `player`. A snapshot stores the resolved unit name and cached helpful and harmful aura records from the Classic-compatible UnitBuff, UnitDebuff, and UnitAura APIs, including spell IDs when the client returns them. Reminder refreshes build one snapshot per relevant unit and reuse it for all name or spell-ID aura lookups in that refresh.
